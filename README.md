...
**[`docs/setup.md`](docs/setup.md)** — دليل مُرتَّب من مستودعٍ على GitHub إلى
مراسلة صادرة مطبوعة، مع سكربتات تهيئة وفحصٍ من ١٣ بندًا يقول لك إن كان النظام
مُهيّأ فعلًا أم لا.

```
scripts/setup/bootstrap.sql   تهيئة المؤسسة والمدير الأول
scripts/setup/harden.sql      إغلاق التسجيل ودعوة الأعضاء
scripts/setup/verify.sql      فحص التهيئة — كل سطر يجب أن يكون PASS
```

> ⚠️ **الترتيب ليس تفصيلًا.** الهجرة `0009` هي التي تُنشئ العضويات، وتعمل على
> المستخدمين الموجودين **وقت تنفيذها**. فتطبيق الهجرات على مشروع فارغ يتركك بلا
> عضوية ولا مدير — ويُصلَح بإعادة تنفيذها بعد أول تسجيل دخول. التفصيل في الدليل.
...
2. افتح **SQL Editor** ونفّذ ملفات الهجرة **بالترتيب**:
   ...
   | 18 | `supabase/migrations/0018_workflow.sql` | آلة الحالة والانتقالات المحمية والتوقيع |
   | 19 | `supabase/migrations/0019_delegation.sql` | التفويض المؤقت |
   | 20 | `supabase/migrations/0020_rls_phase4.sql` | RLS لسير العمل والتفويض |
   | 21 | `supabase/migrations/0021_agent_foundation.sql` | أدوات الوكيل وحصته وتتبّعه |
...
`npm run test` — **303 اختبارًا** تغطي:
...
بالإضافة إلى **٢٢٨ اختبارًا للتكامل** على PostgreSQL حقيقي — حد المعدّل، بوابة التسجيل،
عزل المؤسسات والأدوار والنطاقات، سجل التدقيق، التصنيف الأمني، ذرّية أرقام المراسلات،
الإحالات والمرفقات، وسير العمل والاعتماد والتوقيع والتفويض، وأن الوكيل لا يرى ما لا يراه
المستخدم، وأن التقارير والبحث والتحقق العلني لا تتجاوز شيئًا من ذلك. معظمها **سلبي**: «هذا
المستخدم يجب ألا…». تُتخطى تلقائيًا بلا `QALAM_TEST_DATABASE_URL`.

ومولّد رمز QR مُختبَر مقابل **متجهات مرجعية** ولّدتها مكتبة مستقلة، محفوظة في المستودع
فلا يحتاج التشغيل إلى شبكة.
...
**أُنجز منذ V1:** المؤسسات والهيكل والأدوار والصلاحيات ولوحة الإدارة وسجل التدقيق (المرحلة ٢) · الوارد والصادر وأرقام المراسلات والتصنيف الأمني والإحالات والمرفقات والإشعارات (المرحلة ٣) · سير العمل والمراجعة والاعتماد والتوقيع وفصل المهام والتفويض وصندوق العمل (المرحلة ٤ — راجع [`docs/workflow.md`](docs/workflow.md)) · وكيل قلم بأدوات قراءة ودفاعات حقن الأوامر (المرحلة ٥ — راجع [`docs/agent.md`](docs/agent.md)) · المساعد الصوتي بالعربية مع منع الأوامر الحساسة (المرحلة ٦ — راجع [`docs/voice.md`](docs/voice.md)) · التقارير والبحث العربي والوثيقة النهائية والتحقق العلني بـQR وهوية المؤسسة وحوكمة القوالب (المرحلة ٧ — راجع [`docs/reports.md`](docs/reports.md) و[`docs/operations.md`](docs/operations.md)) · شاشة إعدادات المراسلة: مستويات التصنيف وصيغة أرقام المراسلات، بحراسةٍ في القاعدة تمنع صيغةً بلا تسلسل وتُسجّل كل تغيير رتبة بوصفه تغييرًا في الوصول.

> ⚠️ **ملفات Storage ليست ضمن النسخة الاحتياطية التلقائية لقاعدة Supabase.** استعادةٌ من
> نسخة القاعدة وحدها تُعيد صفوف المرفقات دون ملفاتها. التفصيل والخيارات في
